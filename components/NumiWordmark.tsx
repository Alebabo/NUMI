...
export default function NumiWordmark({ size = "sm", href, className = "", variant = "dark", logoOpacity }: NumiWordmarkProps) {
  const s = sizes[size];
  const wordmarkColor = variant === "light" ? "text-white" : "text-gray-900";
  const filter = variant === "light" ? "brightness-0 invert" : "brightness-0";
  ...
  const inner = (
    <span className={`inline-flex items-center gap-2 ${className}`} aria-label="Numi">
      <Image
        ...
        style={{ height: s.logo, flexShrink: 0, opacity: logoOpacity ?? 1 }}
        aria-hidden="true"
      />
